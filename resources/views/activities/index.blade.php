...
@section('content')
    ...
    <div x-data="{
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <form action="{{ url()->current() }}" method="GET" class="w-full sm:w-auto">
                        <select name="status" onchange="this.form.submit()"
                            class="w-full sm:w-48 appearance-none rounded-lg border border-gray-300 bg-white py-2.5 px-4 text-sm font-medium text-gray-700 outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white cursor-pointer">
                        ...
                        <tbody>
                            @forelse($activities as $activity)
                                <tr class="border-b border-gray-100 dark:border-gray-800 last:border-0 hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                                    ...
                                    {{-- KOLOM TANGGAL MULAI (Format: Tanggal atas, Jam bawah) --}}
                                    <td class="px-5 py-4">
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            {{ \Carbon\Carbon::parse($activity->start_date)->format('H:i') }} WIB
                                        </div>
                                    </td>

                                    {{-- KOLOM TANGGAL SELESAI (Format: Tanggal atas, Jam bawah) --}}
                                    <td class="px-5 py-4">
                                        @if($activity->end_date)
                                            ...
                                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                                {{ \Carbon\Carbon::parse($activity->end_date)->format('H:i') }} WIB
                                            </div>
                                        @else
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-5 py-10 text-center text-gray-500">Belum ada data {{ $pageType }}.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        {{-- [Bagian Modal Tambah & Edit silakan biarkan seperti kode terakhir Anda yang sudah berfungsi] --}}
        
    </div>


        {{-- MODAL TAMBAH (Tetap Sama) --}}
        <div x-show="showAddModal" style="display: none;" class="fixed inset-0 z-[999999] flex items-center justify-center bg-gray-900/50 backdrop-blur-sm px-4">
            <div class="w-full max-w-lg rounded-xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-gray-700 dark:bg-gray-800" @click.outside="showAddModal = false">
                ...
                <form action="{{ $unitId ? route('activities.store', $unitId) : '#' }}" method="POST">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" required class="w-full rounded-lg border border-gray-300 bg-transparent py-2.5 px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:border-[#3C50E0]" />
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Status</label>
                            <select name="status" class="w-full rounded-lg border border-gray-300 bg-transparent py-2.5 px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:border-[#3C50E0]">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="datetime-local" name="start_date" required class="w-full rounded-lg border border-gray-300 bg-transparent py-2.5 px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:border-[#3C50E0]" />
                            </div>
                            <div>
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Selesai</label>
                                <input type="datetime-local" name="end_date" class="w-full rounded-lg border border-gray-300 bg-transparent py-2.5 px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:border-[#3C50E0]" />
                            </div>
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Lokasi</label>
                            <input type="text" name="location" placeholder="Contoh: Aula" class="w-full rounded-lg border border-gray-300 bg-transparent py-2.5 px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:border-[#3C50E0]" />
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Deskripsi</label>
                            <textarea name="description" rows="3" class="w-full rounded-lg border border-gray-300 bg-transparent py-2.5 px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:border-[#3C50E0]"></textarea>
                        </div>
                    </div>

                    <div class="mt-8 flex justify-end gap-3">
                        <button type="button" @click="showAddModal = false" class="rounded-lg border border-gray-300 py-2 px-5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300">Batal</button>
                        <button type="submit" class="rounded-lg bg-[#3C50E0] py-2 px-5 text-sm font-medium text-white hover:bg-opacity-90 shadow-sm">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="showDetailModal" style="display: none;" class="fixed inset-0 z-[999999] flex items-center justify-center bg-gray-900/50 backdrop-blur-sm px-4">
            <div class="w-full max-w-lg rounded-xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-gray-700 dark:bg-gray-800" @click.outside="showDetailModal = false">
                <div class="mb-6 flex items-center justify-between border-b border-gray-100 pb-4 dark:border-gray-700">
                ...
                <form x-bind:action="'{{ url('activities') }}/' + (selectedItem ? selectedItem.id : '')" method="POST">
                    ...
                    <template x-if="selectedItem">
                        <div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Judul</label>
                                    <input type="text" name="title" x-model="selectedItem.title" required class="w-full rounded-lg border border-gray-300 bg-transparent py-2.5 px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white" />
                                </div>
                                <div>
                                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Status</label>
                                    <select name="status" x-model="selectedItem.status" class="w-full rounded-lg border border-gray-300 bg-transparent py-2.5 px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white">
                                        <option value="pending">Pending</option>
                                        <option value="progress">Progress</option>
                                        <option value="completed">Completed</option>
                                    </select>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Mulai</label>
                                        <input type="datetime-local" name="start_date" :value="formatDate(selectedItem.start_date)" @input="selectedItem.start_date = $event.target.value" required class="w-full rounded-lg border border-gray-300 bg-transparent py-2.5 px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white" />
                                    </div>
                                    <div>
                                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Selesai</label>
                                        <input type="datetime-local" name="end_date" :value="formatDate(selectedItem.end_date)" @input="selectedItem.end_date = $event.target.value" class="w-full rounded-lg border border-gray-300 bg-transparent py-2.5 px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white" />
                                    </div>
                                </div>
                                <div>
                                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Lokasi</label>
                                    <input type="text" name="location" x-model="selectedItem.location" class="w-full rounded-lg border border-gray-300 bg-transparent py-2.5 px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white" />
                                </div>
                                <div>
                                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Deskripsi</label>
                                    <textarea name="description" rows="3" x-model="selectedItem.description" class="w-full rounded-lg border border-gray-300 bg-transparent py-2.5 px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white"></textarea>
                                </div>
                            </div>
                        </div>
                    </template>
                    ...
                    <div class="mt-8 flex justify-between items-center">
                        <button type="button" @click="if(confirm('Yakin ingin menghapus?')) { document.getElementById('delete-form-' + selectedItem.id).submit(); }" class="text-red-500 hover:text-red-700 font-medium text-sm flex items-center gap-1">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                            Hapus
                        </button>
                        <div class="flex gap-3">
                            <button type="button" @click="showDetailModal = false" class="rounded-lg border border-gray-300 py-2 px-5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300">Batal</button>
                            <button type="submit" class="rounded-lg bg-[#3C50E0] py-2 px-5 text-sm font-medium text-white hover:bg-opacity-90 shadow-sm">Update</button>
                        </div>
                    </div>
                </form>
